    ...
    <div className="flex items-center justify-center bg-gray-100 dark:bg-black p-6 min-h-screen relative">
      {isLoading && (
        <div className="fixed inset-0 flex flex-col justify-center items-center bg-black bg-opacity-70 z-50">
          <p className="text-white text-lg font-semibold mb-4">Please wait, uploading your post...</p>
        ...
        <form onSubmit={handleCreatePost}>
              ...
              {mediaFiles.map((media, index) => (
                <SwiperSlide key={index} className="relative">
                  {media.file.type.startsWith('image') ? (
                    <img src={media.preview} alt="Preview" className="w-full h-96 object-contain rounded-lg" />
                  ) : (
                    <video src={media.preview} className="w-full h-96 object-contain rounded-lg" controls />
                  )}
                  <button
                    type="button"
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 bg-blue-600 text-white text-lg py-3 rounded-lg ${
              mediaFiles.length === 0 || isLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'
            }`}
